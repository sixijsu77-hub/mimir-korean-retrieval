...
## "The Korean BM25 baseline" is not one configuration

`mteb/baseline-bm25s` carries **27 results whose language list includes Korean** across 24
tasks (25 Korean-only, 2 Korean↔English), and **not one of them records a version at
2.18.12 or later** — the release where Korean first gets its own tokenizer. What their
recorded versions imply:

| Recorded version | Results | What that version does with Korean |
|---|---|---|
| ≤ 2.14.1 | 14 | English stopwords + English Snowball stemmer, word split |
| 2.14.2 – 2.18.11 | 13 | word split, no stopwords, frequency stopwords |
| ≥ 2.18.12 | **0** | character unigram, frequency stopwords |

Inventory: [`results/korean_baseline_inventory.jsonl`](results/korean_baseline_inventory.jsonl),
regenerated by `scripts/count_korean_baselines.py`. Four were reproduced here, and they
need **two different configurations** — and one of them does not match its own recorded
version:
...
**every** language — that is what the first three rows reproduce under, matching what their
recorded versions do. **AutoRAGRetrieval does not.** Its score needs the character path,
which first exists at 2.18.12, while its file records 2.12.30 — a version with no language
table, no character tokenizer and no frequency-stopword step. So when a result was
*actually* produced cannot be read off these files, which is the substance of the report
filed upstream.
